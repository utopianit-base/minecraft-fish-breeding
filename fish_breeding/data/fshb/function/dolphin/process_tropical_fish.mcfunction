...
execute unless score #dolphins_breed fb.config matches 1 run tellraw @a [{"text":"[DEBUG] Breeding disabled, exiting","color":"red"}]
...
execute as @e[type=item,nbt={Item:{id:"minecraft:tropical_fish"}},tag=!db.tropical_fish_used] at @s run tellraw @a [{"text":"[DEBUG] Found tropical fish item, calling find_dolphin_pairs","color":"yellow"}]
