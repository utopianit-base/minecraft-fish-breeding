...
execute as @e[type=minecraft:pufferfish,distance=..5,tag=fb.tracked,tag=!fb.breeding,scores={fb.breed_cooldown=0,fb.mature_cooldown=0},limit=1,sort=nearest] run tag @s add fb.parent1
...
execute as @e[type=minecraft:pufferfish,tag=fb.newborn,limit=1] run scoreboard players set @s fb.breed_cooldown 0
