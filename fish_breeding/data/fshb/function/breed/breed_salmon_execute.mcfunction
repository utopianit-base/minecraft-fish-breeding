...
execute as @e[type=minecraft:salmon,distance=..5,tag=fb.tracked,tag=!fb.breeding,scores={fb.breed_cooldown=0,fb.mature_cooldown=0},limit=1,sort=nearest] run tag @s add fb.parent1
execute as @e[tag=fb.parent1] run tag @s add fb.breeding

# Tag the second eligible salmon as parent2
execute as @e[type=minecraft:salmon,distance=..5,tag=fb.tracked,tag=!fb.breeding,scores={fb.breed_cooldown=0,fb.mature_cooldown=0},limit=1,sort=nearest] run tag @s add fb.parent2
execute as @e[tag=fb.parent2] run tag @s add fb.breeding

# Spawn offspring at center of block to avoid suffocation
execute at @s align xyz run summon minecraft:salmon ~0.5 ~0.5 ~0.5 {Tags:["fb.tracked","fb.juvenile","fb.newborn"]}

# Set maturation cooldown on offspring
execute as @e[type=minecraft:salmon,tag=fb.newborn,limit=1] run scoreboard players operation @s fb.mature_cooldown = #mature_cooldown fb.config
execute as @e[type=minecraft:salmon,tag=fb.newborn,limit=1] run scoreboard players set @s fb.breed_cooldown 0
