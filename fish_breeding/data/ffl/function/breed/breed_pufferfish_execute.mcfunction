# Fish Breeding - Execute Pufferfish Breeding
# Spawns offspring and applies effects

# Tag the first eligible pufferfish as parent1
execute as @e[type=minecraft:pufferfish,distance=..5,tag=ffl.tracked,tag=!ffl.breeding,scores={ffl.breed_cooldown=0,ffl.mature_cooldown=0},limit=1,sort=nearest] run tag @s add ffl.parent1
execute as @e[tag=ffl.parent1] run tag @s add ffl.breeding

# Tag the second eligible pufferfish as parent2
execute as @e[type=minecraft:pufferfish,distance=..5,tag=ffl.tracked,tag=!ffl.breeding,scores={ffl.breed_cooldown=0,ffl.mature_cooldown=0},limit=1,sort=nearest] run tag @s add ffl.parent2
execute as @e[tag=ffl.parent2] run tag @s add ffl.breeding

# Spawn offspring at center of block to avoid suffocation
execute at @s align xyz run summon minecraft:pufferfish ~0.5 ~0.5 ~0.5 {Tags:["ffl.tracked","ffl.juvenile","ffl.newborn"]}

# Set maturation cooldown on offspring
execute as @e[type=minecraft:pufferfish,tag=ffl.newborn,limit=1] run scoreboard players operation @s ffl.mature_cooldown = #mature_cooldown ffl.config
execute as @e[type=minecraft:pufferfish,tag=ffl.newborn,limit=1] run scoreboard players set @s ffl.breed_cooldown 0
